...
/**
 * An service for downloading all the updates of the server.
 */
public class SunshineService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        Log.i(LOG_TAG, "In Sunshine service");
        String location = intent.getStringExtra(LOCATION_PARAM);
    ...
    private String[] getWeatherDataFromJson(String forecastJsonStr, int numDays, String locationSetting)
        ...
        for(int i = 0; i < weatherArray.length(); i++) {
            ...
            if(cVVector.size() > 0){
                ...
                final int rowsInserted = getApplicationContext().getContentResolver().
...
*/
    ...
    private String formatHighLows(double high, double low) {
        ...
        final String unitType = defaultSharedPreferences.getString(getApplicationContext().getString(R.string.pref_unit_key),
                getApplicationContext().getString(R.string.pref_unit_default_value));

        if (unitType.equals(getApplicationContext().getString(R.string.pref_unit_imperial))) {
            high = (high * 1.8) + 32;
            low = (low * 1.8) + 32;
        } else if (!unitType.equals(getApplicationContext().getString(R.string.pref_unit_metric))) {
    ...
    private long addLocation(String locationSettings, String cityName, double lat, double lon){
        ...
        long id = 0;
        final Cursor query = getApplicationContext().getContentResolver().
        ...
        if(query.moveToFirst()){
            ...
            final Uri insert = getApplicationContext().getContentResolver()
     ...
     static public class AlarmReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            final String location = intent.getStringExtra(SunshineService.LOCATION_PARAM);
            Intent sunshineServiceIntent = new Intent(context, SunshineService.class);
            sunshineServiceIntent.putExtra(SunshineService.LOCATION_PARAM, location);
